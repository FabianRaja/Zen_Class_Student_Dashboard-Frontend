...
export default function LeaveApplicationsPage(){
    ...
    return(
        ...
        <MainSpace>
             ...
             <h5 className="card-title justify-content-center text-center text-neutral">Number of Applications Submitted - {userDetails.leaves.count}</h5>
             <br/>
                <div className="card">
                <div className="card-body">
                <form className="leave-application-submission-form" onSubmit={handleSubmit}>
                <label >From</label>
                <input type="date" placeholder="Type here" name="from" onBlur={handleBlur} value={values.from} onChange={handleChange} className="input text-center input-bordered w-full max-w-xs text-neutral" />
                {touched.from && errors.from?(<div className="text-error">{errors.from}</div>):""}
                <label >To</label>
                <input type="date" placeholder="Type here" name="to" onBlur={handleBlur}  value={values.to} onChange={handleChange} className="input text-center input-bordered w-full max-w-xs text-neutral" />  
                {touched.to && errors.to?(<div className="text-error">{errors.to}</div>):""}
                <label >Reason</label>
                <input type="text" placeholder="Reason" name="reason" onBlur={handleBlur}  value={values.reason} onChange={handleChange} className="input text-center input-bordered w-full max-w-xs text-neutral" />    
